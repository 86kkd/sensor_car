...
#define I2C_CMD(err, from)                                                     \
  if (err != ESP_OK) {                                                         \
    ESP_LOGE("I2C ERROR: ", "%s, %s", from, esp_err_to_name(err));             \
  }
...
void vAHTStatus() {
  ...
  xI2CRead(&data, 1);
  ESP_LOGI("STATUS", "Bisy bit - %d, calibration bit - %d", readBit(data, 7),
           readBit(data, 3));
}
...
static void vAHTRead() {
  ...
  raw_data[1] = (((uint32_t)(calc_data[1]) << 12)) |
                (((uint32_t)calc_data[2]) << 4) | ((calc_data[3] & 0xF0) >> 4);
  raw_data[2] = ((uint32_t)(calc_data[3] & 0x0F) << 16) |
                ((uint16_t)calc_data[4] << 8) | calc_data[5];

  ESP_LOGI("Measure:", "[t:%f: h: %f] - [%s]",
...
static void vI2CCmd(uint8_t commands[3], int delay) {
  ...
  I2C_CMD(i2c_master_write_byte(cmd, (AHT10_ADDRESS << 1) | I2C_MASTER_WRITE,
                                I2C_MASTER_ACK),
          "Write");
...
static esp_err_t xI2CRead(uint8_t *data, uint8_t len) {
  ...
  I2C_CMD(i2c_master_start(cmd), "Start");
  I2C_CMD(i2c_master_write_byte(cmd, (AHT10_ADDRESS << 1) | I2C_MASTER_READ,
                                I2C_MASTER_ACK),
          "Write");
